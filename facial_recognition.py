...
# make sure to remove .DS_Store file
...
# define check_face function that will be run in a thread
        ...
        try:   
            ...
            if DeepFace.verify(frame, reference_image.copy())['verified']:
                if person_present not in face_matches:
                    ...
                    with open('data_camera.txt', 'w') as file:
...
while True:
    ...
    if ret:
        ...
        if len(face_matches) > 0:
            ...
            if person_num > 1:
                cv2.putText(frame, f"{people} are here", (20, 450), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 3)
